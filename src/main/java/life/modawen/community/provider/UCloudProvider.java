package life.modawen.community.provider;
...
public class UCloudProvider {
    ...
    public String upload(InputStream fileStream, String mineType, String fileName) {
        ...
        String genernatedFileName;
        String[] filePaths = fileName.split("\\.");
        if (filePaths.length > 1) {
            genernatedFileName = UUID.randomUUID().toString() + "." + filePaths[filePaths.length - 1];
        } else {
            ...
            PutObjectResultBean response = UfileClient.object(objectAuthorization, config)
                    ...
                    if(response != null && response.getRetCode() == 0){
                        String url = UfileClient.object(objectAuthorization, config)
                                .getDownloadUrlFromPrivateBucket(genernatedFileName, bucketName, 315360000)
                                .createUrl();
                        return url;
                    }else{
                        throw new CustomizeException(CustomizeErrorCode.FILE_UPLOAD_FAIL);
